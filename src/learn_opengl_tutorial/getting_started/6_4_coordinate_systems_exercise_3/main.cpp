...
int main()
{
    ...
    const auto vertexShaderFilePath = std::filesystem::current_path().append(
      "src/learn_opengl_tutorial/getting_started/6_4_coordinate_systems_exercise_3/vertex_shader.vs");
    ...
    const auto fragmentShaderFilePath = std::filesystem::current_path().append(
      "src/learn_opengl_tutorial/getting_started/6_4_coordinate_systems_exercise_3/fragment_shader.fs");
    ...
    while (!window.shouldClose())
    {
        ...
        projection =
          glm::perspective(glm::radians(45.0f), static_cast<float>(windowWidth) / windowHeight, 0.1f, 100.0f);
        ...
        for (size_t cubePositionIndex{ 0 }; cubePositionIndex < cubePositions.size(); ++cubePositionIndex)
        {
            ...
            // Turn every 3rd box including the 1st one
            if (cubePositionIndex % 3 == 0)
            {
                const auto currentTime = static_cast<float>(glfwGetTime());
                angle = angle + currentTime;
                angle = angle * currentTime;
            }
